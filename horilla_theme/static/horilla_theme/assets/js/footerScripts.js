...
function setAccordionFoldIcon(icon, isOpen) {
    if (!icon) {
        return;
    }
    // Match recruitment pipeline fold icons: open = up, closed = down.
    if (icon.tagName && icon.tagName.toLowerCase() === "ion-icon") {
        icon.setAttribute(
            "name",
